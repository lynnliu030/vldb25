...
from src.prompts.q2p import MOVIES_PROMPT_SHORT, PRODUCTS_PROMPT_SHORT, PDMX_PROMPT_SHORT, BEER_PROMPT_SHORT, BIRD_PROMPT_SHORT
import numpy as np
...
async def call_LLM_UDF(aggregated_df, cols, prompt_prefix: str, dedup: bool = False, guided_choice: List[str] = None, port: int = 8000):
    # If guided choice is None, for Q2, set it to empty list
...
async def single_select(
    ...
    async def execute_llm_task(i, df_partition):
        ...
        llm_time, output_length, result_exploded = await call_LLM_UDF(df_partition, cols, prompt_prefix, dedup, guided_choice, port=port)
    ...
    if result_dfs:
        ...
        if "movie" in dataset_name:
            result_df_combined = result_df_combined.filter(upper(col("results")) == "POSITIVE")
        elif "product" in dataset_name:
            result_df_combined = result_df_combined.filter(upper(col("results")) == "POSITIVE")
        elif "PDMX" in dataset_name:
            result_df_combined = result_df_combined.filter(upper(col("results")) == "YES")
        elif "beer" in dataset_name:
            result_df_combined = result_df_combined.filter(upper(col("results")) == "NO")
        elif "BIRD" in dataset_name:
            result_df_combined = result_df_combined.filter(upper(col("results")) == "YES")
        else:
            raise ValueError(f"Dataset {dataset_name} not supported")
    ...
    if output_path:
        ...
        print(f"Filtered df size: {result_df_combined.count()}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--algo_config",
        default="naive",
        help="Choose naive fixed or greedy algorithm for reordering",
    )
