from src.core.udfs import llm_average_naive, llm_average_dedup
...
from src.prompts.q4p import MOVIES_PROMPT_SHORT, PRODUCTS_PROMPT_SHORT
import numpy as np
        ...
        if dedup:
            result_df = await loop.run_in_executor(
                ...
                lambda: aggregated_df.select(
                    llm_average_dedup(lit(prompt_prefix), col("contexts"), lit(cols), lit(guided_choice), lit(port)).alias("results")
                ),
            )
        else:
            result_df = await loop.run_in_executor(
                pool,
                lambda: aggregated_df.select(
                    llm_average_naive(lit(prompt_prefix), col("contexts"), lit(cols), lit(guided_choice), lit(port)).alias("results")
                ),
            )
...
async def single_select(
    ...
    async def execute_llm_task(i, df_partition):
        ...
        llm_time, output_length, result_exploded = await call_LLM_UDF(
            df_partition, cols, prompt_prefix, dedup, guided_choice=guided_choice, port=port
        )
    ...
    if output_path:
        ...
        final_result = result_df_combined.agg({"results": "avg"}).collect()[0]
        print(f"Average score: {final_result}")
