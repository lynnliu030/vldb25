...
FLAGS = flags.FLAGS
flags.DEFINE_string("account_id", None, "AWS 12-digit account ID")
...
def delete_role(role_name, policy_arn):
    client = boto3.client("iam")
...
def create_iam_role(src_region, dst_regions):
    client = boto3.client("iam")
    bucket_names = [
        bucket_handle(region.split(":")[1]) for region in [src_region] + dst_regions
    ]
    ...
    batch_policy = {
        ...
        "Statement": [
            ...
            },
            {
                "Effect": "Allow",
                "Action": ["s3:PutObject"],
                "Resource": ["arn:aws:s3:::laion-400m-dataset/*"],
            },
            {
                "Effect": "Allow",
                "Action": [
    ...
    response = client.create_policy(
        PolicyName="batch" + policy_name, PolicyDocument=json.dumps(batch_policy)
    )
...
def main(argv):
    ...
    # creat iam role
    role_name, role_arn, policy_arn = create_iam_role(src_region, dst_regions)
    ...
    try:
        resp = client.put_bucket_replication(
            Bucket=src_name, ReplicationConfiguration={"Role": role_arn, "Rules": rules}
        )
        print(resp)
    except Exception as e:
    ...
    target_objects = list(
        buckets[src_region.split(":")[1]].list_objects(prefix=experiment_name)
    )
    print(f"Waiting for len(target_objects) to replicate")
